...
// Rendering V2 uses OpenGL instead of the internal screen buffer
//#define STRB_USE_RENDER_V2
...
Game_mark(Game* game)
{
  if(game) {
    ...
    //if(!NIL_P(game->sprites)) {
    //  rb_gc_mark(game->sprites);
    //}
  }
}
...
InitializeScreen(Game* game)
{
  ...
  Uint32 options = 0;
  //options |= SDL_SWSURFACE | SDL_ASYNCBLIT | SDL_DOUBLEBUF | SDL_OPENGL;
  options = SDL_OPENGL;
...
#ifdef STRB_USE_RENDER_V2
  rb_yield(self);
#endif
#ifndef STRB_USE_RENDER_V2
...
static void
strb_GameUpdateEvents(Game* game)
{
  SDL_Event event;
...
Game_wait(VALUE self)
{
  ...
  Data_Get_Struct(self, Game, game);
  CheckDisposed(game);
  GameTimer* game_timer = &(game->timer);
  Uint32 now;
  while (true) {
    now = SDL_GetTicks();
    Uint32 diff = (now - game_timer->before) * game->frame_rate + game_timer->error;
    if (1000 <= diff) {
      game_timer->error = MIN(diff - 1000, 1000);
      game_timer->before = now;
      break;
    }
    SDL_Delay(1);
  }
  ...
  game_timer->counter++;
  if (1000 <= now - game_timer->before2) {
    game->fps = game_timer->counter * 1000.0 / (now - game_timer->before2);
    game_timer->counter = 0;
    game_timer->before2 = now;//SDL_GetTicks();
  }
...
VALUE
strb_InitializeGame(VALUE rb_mSub)
{
  rb_cGame = rb_define_class_under(rb_mSub, "Game", rb_cObject);
