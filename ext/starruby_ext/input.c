...
typedef struct strb_keyboard_key {
  VALUE rbSymbol;
  SDLKey sdlKey;
  int state;
  struct strb_keyboard_key* next;
} KeyboardKey;
...
Input_mouse_location_eq(VALUE self, VALUE rbValue)
{
  ...
  SDL_WarpMouse(NUM2INT(rb_ary_entry(rbValue, 0)) * windowScale,
                NUM2INT(rb_ary_entry(rbValue, 1)) * windowScale);
