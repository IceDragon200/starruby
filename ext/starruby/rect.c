...
Void strb_RubyToRect(VALUE rbObj, Rect* rect)
{
  switch (TYPE(rbObj)) {
    case T_DATA: {
      strb_CheckObjIsKindOf(rbObj, rb_cRect);
      Rect* src_rect;
    ...
    case T_ARRAY: {
      if (hrbCheckArraySize(rbObj, ==, 4)) {
        rect->x      = hrbArrayEntryAsInt(rbObj, 0);
        rect->y      = hrbArrayEntryAsInt(rbObj, 1);
        rect->width  = hrbArrayEntryAsInt(rbObj, 2);
        rect->height = hrbArrayEntryAsInt(rbObj, 3);
      } else {
...
static VALUE Rect_s_cast(VALUE klass, VALUE rbObj)
{
  Rect* rect;
  VALUE rbRect;
