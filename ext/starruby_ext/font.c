           ...
           VALUE* volatile rbRealFilePath, int* ttcIndex)
{
  ...
  *rbRealFilePath = strb_GetCompletePath(rbFilePathOrName, false);
  ...
  volatile VALUE rbFontNameSymbol = ID2SYM(rb_str_intern(rbFilePathOrName));
...
Font_s_new(int argc, VALUE* argv, VALUE self)
{
  ...
  //if (underline) {
  //  rb_str_cat2(rbHashKey, ";underline=true");
  //} else {
  //  rb_str_cat2(rbHashKey, ";underline=false");
  //}
  ...
  if (!NIL_P(val = rb_hash_aref(rbFontCache, rbHashKey))) {
    ...
    volatile VALUE rbNewFont = rb_funcall(self, rb_intern("allocate"), 0);
    rb_obj_call_init(rbNewFont, 5, args);
