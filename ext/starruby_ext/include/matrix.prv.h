...
#define MATRIX_MATH_MATRIX_BANG_FUNC(namespace, strct, arytype, datatype, rbconv, word, sym, zero_protect) \
  ...
  { \
    strb_AssertObjIsKindOf(self, rb_cMatrixI); \
...
#define MATRIX_MATH_FUNC(namespace, strct, arytype, datatype, rbconv, word, sym, zero_protect) \
  ...
  { \
    ...
    if(using_matrix) \
    { \
      strb_AssertObjIsKindOf(rbObj, rb_c ## strct); \
      Data_Get_Struct(rbObj, MatrixI, src_matrix); \
    } \
    else \
    { \
      src_num = NUM2INT(rbObj); \
    } \
    \
    ...
    trgSize   = NUM2INT(rbTrgSize); \
    startSize = NUM2INT(rbStartSize); \
    endSize   = NUM2INT(rbEndSize); \
    stackSize = trgSize; \
    \
    if(stackSize <= 0) \
    { \
