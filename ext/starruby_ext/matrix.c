...
static void
strb_MatrixI_free(MatrixI *matrix)
{
...
MatrixI_default_bang(VALUE self)
{
  ...
  int32_t default_val = NUM2INT(rb_iv_get(self, "default"));
...
MatrixI_entry_set(int argc, VALUE *argv, VALUE self)
{
  ...
  if(!(index < 0))
    matrix->data->data[index] = NUM2INT(value);
...
MATRIX_MATH_FUNCS(MatrixI, MatrixI, ArrayI, int32_t, NUM2INT);
...
VALUE
strb_InitializeMatrix(VALUE rb_mStarRuby)
{
