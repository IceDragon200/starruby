...
static void
strb_FontRefreshStyle(Font *font)
{
...
Font_initialize(int argc, VALUE* argv, VALUE self)
{
  ...
  if (NIL_P(rbRealFilePath)) {
    char* tmppath = StringValueCStr(rbPath);
...
static VALUE
Font_bold_set(VALUE self, VALUE rbBool)
{
...
static VALUE
Font_italic_set(VALUE self, VALUE rbBool)
{
  rb_check_frozen(self);
  Font* font;
  Data_Get_Struct(self, Font, font);
...
static VALUE
Font_underline_set(VALUE self, VALUE rbBool)
{
  rb_check_frozen(self);
  Font* font;
  Data_Get_Struct(self, Font, font);
